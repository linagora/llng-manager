...
import { updateModuleParams } from "../../features/config/configSlice";
    ...
    <>
      <table>
        ...
        <tbody>
          {Object.keys(data).map((key) => (
            <tr>
              <td>
                <TextField type="text" placeholder={t(key)} value={key} />
              </td>
              <td>
      ...
      {Object.keys(data).map((key) => (
        <Accordion>
          ...
          <span>{t("newCmbOver")}</span>
          <Button className="plus">
            <AddCircleIcon color="success" />
          </Button>
          {TableVars(
            "combover",
            data[key].over as Record<string, string>,
            "aab",
            console.log,
            console.log,
            console.log
